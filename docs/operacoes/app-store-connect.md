...
|-----|-----------|--------------|
| **Pulse Eventos** (app-client) | `com.pulse.ingressos` | Pulse Eventos |
...
| Campo | Valor |
|-------|--------|
| **Merchant ID** | `merchant.com.pulse.ingressos` |
| **App ID** | `com.pulse.ingressos` |
| **Capability** | Apple Pay Payment Processing |

### Passos

1. **Identifiers → Merchant IDs** → criar (ou confirmar) `merchant.com.pulse.ingressos`
2. **Identifiers → App IDs** → `com.pulse.ingressos` → editar → marcar **Apple Pay Payment Processing** → selecionar o Merchant ID acima → salvar
...
> Se o build falhar com erro de provisioning profile, apague profiles antigos de `com.pulse.ingressos` no portal e rode o build de novo.
...
|-------|--------|
| App ID | `com.pulse.ingressos` |
| Services ID (`APPLE_CLIENT_ID`) | `com.pulse.ingressos.auth` |
| Key ID (Sign In with Apple) | confirmar no Apple Developer; não é a API Key do App Store Connect |
