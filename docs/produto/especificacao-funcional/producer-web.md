...
- Modo de entrada obrigatório na criação/edição:
  - `QR_ONLY`: entrada por QR Code, sem transferência obrigatória por biometria.
  - `BIOMETRY_OPTIONAL`: facial com QR fallback; extras de compra multi-ingresso facial ficam pendentes de transferência.
  - `BIOMETRY_ONLY`: apenas biometria; não há QR Code no app.
- Após venda de ingressos, o backend bloqueia alteração do modo de entrada para preservar titulares, comunicação e operação de portaria.
