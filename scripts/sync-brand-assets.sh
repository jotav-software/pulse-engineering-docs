#!/usr/bin/env bash
# Sincroniza brand assets canônicos de pulse-engineering-docs/brand/assets/ para os repos de código.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
DOCS="$(cd "$(dirname "$0")/.." && pwd)"
ASSETS="$DOCS/brand/assets"
...
proto="$ROOT/producer-web/prototypes/landing-page/assets"
if [[ -d "$proto" ]]; then
