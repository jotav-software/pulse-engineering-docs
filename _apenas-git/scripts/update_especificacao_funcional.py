...
def ecosystem_blocks() -> list[tuple[str, str]]:
    return [
        ...
        (
            ...
            "produto/regras-negocio/global-business-rules.md, produto/regras-negocio/checkout-compliance.md (pulse-engineering-docs).",
        ),
        ...
        (
            ...
            "ingresso só emitido após status PAID (produto/regras-negocio/global-business-rules.md). "
...
def build_client_module_table(doc: Document) -> None:
    ...
    doc.add_paragraph(
        ...
        "(ver produto/regras-negocio/payout-policies.md — canônico D+1)."
    )
    ...
    for p in doc.paragraphs[:8]:
        ...
        if "Documento base para definição" in p.text or "Documento único" in p.text:
            p.text = (
                ...
                "produto/regras-negocio/global-business-rules.md, produto/regras-negocio/payout-policies.md, código (apps + backend)."
            )
    ...
    if doc.tables:
        ...
        if len(t0.rows) >= 4:
            ...
            t0.rows[3].cells[1].text = (
                "produto/acesso/rbac.md, global-business-rules.md, checkout-compliance.md"
            )
        if len(doc.tables) > 1:
