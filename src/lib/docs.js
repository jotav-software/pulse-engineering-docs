...
const DOCS_SECTIONS = [
  { id: 'arquitetura', label: 'Arquitetura', description: 'Decisões (ADR) e visão sistêmica' },
  { id: 'produto', label: 'Produto', description: 'Specs, regras, acesso e biometria' },
  { id: 'negocio', label: 'Negócio', description: 'Contratos, LGPD, comercial e GTM' },
  { id: 'operacoes', label: 'Operações', description: 'Infra, variáveis, deploy, CDN' },
  { id: 'padroes', label: 'Padrões', description: 'Padrões de código, testes, backlog' },
];

const SECTION_IDS = new Set(DOCS_SECTIONS.map((s) => s.id));
const SKIP_DIRS = new Set(['node_modules', '.git', 'public', '_apenas-git', 'src']);
...
function isAllowedDocPath(absolutePath) {
  ...
  const top = parts[0];
  if (top === 'README.md') {
    return false; // README is handled separately or not served via /docs route directly
  }
...
<body>
  <div class="layout">
    <aside class="sidebar">
      ...
      <div class="subtitle">Documentação interna</div>
      ${sidebarHtml}
    </aside>
    <main class="content">
