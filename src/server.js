...
app.use('/assets', express.static(path.join(PUBLIC_ROOT, 'assets'), staticOptions));

// Protected — HTML kits, tokens.css, brief
app.use('/kit', requireInternalAuth, express.static(path.join(PUBLIC_ROOT, 'kits'), staticOptions));

app.get('/brand-kit-brief.md', requireInternalAuth, (_req, res) => {
  res.sendFile(path.join(REPO_ROOT, 'docs', 'negocio', 'brand-kit-brief.md'));
});
