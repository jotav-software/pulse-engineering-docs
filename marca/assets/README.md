...
## Por que `marca/assets/` (e não `logos/`)

O kit exportado (`pulse-brand-kit-ultima-versao`) inclui muito mais que logotipos — ícones de app, telas de splash, avatares para Instagram, paleta de cores e variantes contornadas. O nome `assets/` reflete essa amplitude e evita confusão com a pasta `marca/kits/` (HTMLs interativos de apresentação).

| Pasta | Conteúdo |
|---|---|
| `marca/assets/` | Arquivos exportados (PNG/SVG) — **fonte canônica para apps e CDN** |
| `marca/kits/` | Kits HTML (pitch, Instagram, brand guide) + `tokens.css` |
| `_apenas-git/capturas-marca/` | Capturas de tela do app para marketing |
